###############################################################################################################
# Import-CdmManagedComputer - Import Computer profile to Zone using CSV format
#
...
<#
.SYNOPSIS
This Cmdlet is intend to import Computers to Zone using CSV data

.DESCRIPTION
This Cmdlet will import one or more Computer profile to one or more Centrify hierarchical Zone by using data stored in a CSV formated file.
...
.PARAMETER Force
Force the import of the Computer profile if any existing.
...
(
	...
	[Parameter(Mandatory = $false, HelpMessage = "Force the import of the Computer profile if any existing.")]
	[Switch]$Force,
...
# Import Computers in target Zone
$index = 1
$Data | ForEach-Object {
	if ($Progress.IsPresent)
	{
		...
		$OverallProgressActivity	= ("Import Computers [{0}/{1}]" -f $index, $Count)
	...
	if ($Force.IsPresent)
	{
		$CmdletParameters.Force = $true
	}

	# Create Computer
	try
	{
		$CdmManagedComputer = New-CdmManagedComputer @CmdletParameters
	}
	catch [System.ApplicationException]
	{
		if ($_.Exception.Message -match "The object already exists.")
		{
			# Cannot create AD Computer object that already exists when prepapring UNIX Computer
			Write-Warning ("Failed to create computer '{0}'. The object already exists." -f $CmdletParameters.Name)
		}
		else
		{
			# Unknown exception
