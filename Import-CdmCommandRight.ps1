###############################################################################################################
# Import-CdmCommandRight - Import Command Right using CSV format
#
...
<#
.SYNOPSIS
This Cmdlet is intend to import Command Right(s) using CSV data

.DESCRIPTION
This Cmdlet will import one or more Command Right(s) in one or more Zone(s) by using data stored in a CSV formated file and associated them to an existing Role if defined. 
...
$Data | ForEach-Object {
	if ($Progress.IsPresent)
	{
		...
		$OverallProgressActivity	= ("Import Command Right [{0}/{1}]" -f $index, $Count)
	...
	if (-not [System.String]::IsNullOrEmpty($_.AllowNested) -and $_.AllowNested -eq "true")
	{
		$CmdletParameters.AllowNested = $true
	}
	else
	{
		$CmdletParameters.AllowNested = $false
	}
	...
	try
	{
		...
		$CdmRole = $CmdletParameters.Role
		$CmdletParameters.Remove("Role")
		$CdmRight = New-CdmCommandRight @CmdletParameters		
	}
	catch [System.ApplicationException]
	{
		if ($_.Exception.Message -match "A command right with the name .* already exists")
		{
			# Get existing CdmRight
			$CdmRight = Get-CdmCommandRight -Zone $CdmZone -Name $CmdletParameters.Name
			
			if ($Force.IsPresent)
			{
				# Try to overwrite object definition
				$CmdletParameters.Remove("Zone")
				Write-Debug ("Overwriting existing command right '{0}'" -f $CmdletParameters.Name)
				
				# Set Object
				$CdmRight | Set-CdmCommandRight @CmdletParameters
			}
			else
			{
				# Cannot create Command Right that already exists
	...
	# If Role is given, add Command Right to the Role
	if (-not [System.String]::IsNullOrEmpty($CdmRole))
	{
		Add-CdmCommandRight -Role (Get-CdmRole -Zone $CdmZone -Name $CdmRole) -Right $CdmRight
	}
