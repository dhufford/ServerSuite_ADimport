###############################################################################################################
# Import-CdmComputerRole - Import Computer Role using CSV format
#
...
<#
.SYNOPSIS
This Cmdlet is intend to import Computer Role(s) using CSV data

.DESCRIPTION
This Cmdlet will import one or more Computer Role(s) in one or more Zone(s) by using data stored in a CSV formated file. 
...
$Data | ForEach-Object {
	if ($Progress.IsPresent)
	{
		...
		$OverallProgressActivity	= ("Import Computer Role [{0}/{1}]" -f $index, $Count)
	...
	if ($ADGroup -eq [Void]$null -and $CreateADGroup.IsPresent)
	{
		...
		try
		{
			$ADGroup = New-ADGroup -GroupCategory "Security" -GroupScope "DomainLocal" -Path $_.Container -Name $ADGroupName -PassThru
		}
		catch
		{
	...
	try
	{
		$CdmComputerRole = New-CdmComputerRole @CmdletParameters
	}
	catch [System.ApplicationException]
	{
		if ($_.Exception.Message -match "Computer role .* already exists")
		{
			# Cannot create Computer Role that already exists
	...
	try
	{
		if ($ADGroup -ne [Void]$null)
		{
			foreach ($Member in ($_.Members -Split ','))
			{
				...
				$ComputerProfile = Get-CdmManagedComputer -Name $Member
				if ($ComputerProfile -ne [Void]$null)
				{
					# Add corresponding AD User to AD Group
					Add-ADGroupMember -Identity $ADGroup -Members $ComputerProfile.Computer.DistinguishedName
				}
				else
				{
					# Computer Profile not found
					Write-Warning ("Failed to get Computer Profile for computer {0}. This computer will not be added to group {1}." -f $Member, $CmdletParameters.Group)
				}
			}
		}
	}
